...
for _p in (_HERE, os.path.join(_HERE, "tacotron2")):
...
# ── load Tacotron 2 ───────────────────────────────────────────────────────────
_CKPT_CANDIDATES = [
    os.path.join(_HERE, "speaker_omani_epoch_0360.pth"),
    os.path.join(_HERE, "checkpoints_omani", "B", "speaker_B_last.pth"),
    os.path.join(_HERE, "speaker_B_last.pth"),
]
_TACOTRON_PATH = next(
...
hifigan_model = None
_asc_path = os.path.join(_HERE, "hifigan-asc.pth")
...
if hifigan_model is None:
    _WR_DIR = os.path.join(_HERE, "wavernn_checkpoints", "checkpoints")
